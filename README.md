...
means a person is reviewing the PR.
...
means work has been chosen for clarification and planning. Plan approval is a separate human
decision. The queue has no time-boxed sprints or capacity planning.
...
![Demo board: backlog, clarification, plan approval and review queues](docs/screenshots/01-board-intake.png)
...
- [Verification](docs/verification.md): test evidence and verification limits.
...
`codex exec` makes the agent invocation visible and portable. Publication runs in separate
jobs with its own credentials; the model key is scoped to the invocation, and validation
runs in a fresh job. The example is an educational starting point for adapting these boundaries. [Official guidance](https://learn.chatgpt.com/docs/non-interactive-mode).
